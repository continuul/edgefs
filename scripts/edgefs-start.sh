...
corosync_periodic_check_ips() {
	local prevIPS=""
	local ips=""
	while true; do
		sleep 30
		if test "x$KUBERNETES_SERVICE_HOST" != x; then
			ips="`k8s_list_pods | grep podIP | md5sum | awk '{print $1}'`"
		fi
		if test "x$ips" != x -a "x$ips" != "x$prevIPS"; then
			if is_leader || is_gateway; then
