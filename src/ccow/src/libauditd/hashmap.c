...
int hashmap_init(int initial_size, hashmap **map) {
    ...
    // Round up to power of 2
        ...
        int most_sig_bit = 0;
        for (uint32_t idx= 0; idx < sizeof(initial_size)*8; idx++) {
    ...
    // Allocate the map
    hashmap *m = je_calloc(1, sizeof(hashmap));
    m->table_size = initial_size;
    m->max_size = MAX_CAPACITY * initial_size;

    // Allocate the table
    m->table = (hashmap_entry*)je_calloc(initial_size, sizeof(hashmap_entry));
...
static int hashmap_insert_table(hashmap_entry *table, int table_size, char *key, int key_len,
    ...
    // since it is empty
    if (last_entry == NULL) {
        entry->key = (should_dup) ? je_strdup(key) : key;
        entry->value = value;

    // We have a last value, need to link against it
    } else {
        entry = je_calloc(1, sizeof(hashmap_entry));
        entry->key = (should_dup) ? je_strdup(key) : key;
        entry->value = value;
...
static void hashmap_double_size(hashmap *map) {
    ...
    // Allocate the table
    hashmap_entry *new_table = (hashmap_entry*)je_calloc(new_size, sizeof(hashmap_entry));
