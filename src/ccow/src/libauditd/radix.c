...
int radix_insert(radix_tree *t, char *key, void **value) {
    ...
    do {
        ...
        if (!search || *search == 0) {
            ...
            if (leaf) {
                ...
                *value = old;
                ...
                // Add a new node
                leaf = je_malloc(sizeof(radix_leaf));
                n->edges[0] = leaf;
                leaf->key = key ? je_strdup(key) : NULL;
                leaf->value = *value;
                return 0;
            }
        }
        ...
        if (!n) {
            child = parent->edges[(int)*search] = je_calloc(1, sizeof(radix_node));
            leaf = child->edges[0] = je_malloc(sizeof(radix_leaf));
            leaf->key = je_strdup(key);
            leaf->value = *value;
        ...
        // If we share a sub-set, we need to split the nodes
            ...
            child = je_calloc(1, sizeof(radix_node));
            ...
            n = je_calloc(1, sizeof(radix_node));
