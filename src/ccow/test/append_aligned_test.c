...
#define TEST_BUCKET_NAME        "aligned-obj-bucket"
#define TEST_FILE_OID		"file1"
#define TEST_FILE_PART1		"file1.1"
#define TEST_FILE_PART2		"file1.2"
#define TEST_FILE_PART3		"file1.3"
...
static int
simple_put_aligned_file(void)
{
	int err = 0;
	err = system("/bin/dd if=/dev/urandom of=file1.1 bs=1M count=1");
	assert_int_equal(err, 0);
	err = system("/bin/dd if=/dev/urandom of=file1.2 bs=1M count=1");
	assert_int_equal(err, 0);
	err = system("/bin/dd if=/dev/urandom of=file1.3 bs=1M count=1");
	assert_int_equal(err, 0);
	err = system("/bin/cat file1.1 file1.2 file1.3 > src_obj");
	assert_int_equal(err, 0);
	...
	for (int part = 0; part < total_parts; part++) {
		...
		for (size_t i = 0; i < iovcnt; i++) {
			...
			assert_int_equal(bytes, block_size);
		}
		ccow_completion_t c;
		err = ccow_create_completion(cl, NULL, NULL, 1, &c);
		assert_int_equal(err, 0);
		...
		err = ccow_put(bucket_name, strlen(bucket_name) + 1, parts[part],
			strlen(parts[part]) + 1, c, iov, iovcnt, 0);
		assert_int_equal(err, 0);
		err = ccow_wait(c, -1);
		assert_int_equal(err, 0);
		close(fd);
...
static void
append_objects_aligned(void **state)
{
	simple_put_aligned_file();
	ccow_completion_t c;
	int total_parts = 3;
	...
	int err = ccow_copy_objects(cluster_name, strlen(cluster_name) + 1, tenant_name,
	      ...
	      object_name, strlen(object_name) + 1, iov, 3, cl);
	assert_int_equal(err, 0);
	err = ccow_create_completion(cl, NULL, NULL, 1, &c);
	simple_get_file();
...
main(int argc, char *argv[])
{
	...
	const UnitTest get_tests[] = {
		...
		unit_test(append_objects_aligned),
