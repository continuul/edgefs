...
#define SBTM_BATCH_SIZE 10

uint64_t SBTM_OFF_MAX = 1024 * 1024 * 1024L;
uint64_t SBTM_OFF_BUF[SBTM_BATCH_SIZE];
...
#define STRESS_TEST_BID		"stress-btreemap-stream-test-bucket"
#define STRESS_TEST_OID		"stress-btreemap-stream-test-object"
uint64_t genid = 0;
...
usage(void)
{
	printf("\n"
	       "USAGE:\n"
	       "     ./stress_btreemap_stream_test [-h] [-c chunk_size] [-i iteratations] \n"
	       "          [-m max_offset] [-o btree_order] [-r] [-t minutes]\n"
	       "\n"
...
int STREAM_SIZE = 2;
int STREAM_IDX  = 0;
ccow_completion_t STREAM_COMP;

/*
 * sbtm_create_completion
 *
 * Create a stream completion.
 */
static void
sbtm_create_completion(ccow_completion_t * c, ccow_t tc, void * arg,
    ccow_callback_t cb_async)
{
	int err = 0;

	//err = ccow_create_completion(tc, arg, cb_async, 1, c);
	err = ccow_create_stream_completion(tc, NULL, NULL, 2, c,
	    STRESS_TEST_BID, strlen(STRESS_TEST_BID) + 1,
	    STRESS_TEST_OID, strlen(STRESS_TEST_OID) + 1,
	    &genid, 0, NULL);
...
sbtm_finalize(void)
{
	...
	err = ccow_finalize(STREAM_COMP, NULL);
	if (err != 0) {
		printf("ccow_finalize returned error status %d \n", err);
	}
	assert_int_equal(err, 0);
	STREAM_IDX = 0;
}
...
static void
btmt_stream_get(ccow_t tc, char *bid, char *oid, struct iovec *iov, size_t iovcnt,
	 ...
	 int expected)
{
	...
	if (STREAM_IDX == 0) {
		sbtm_create_completion(&STREAM_COMP, tc, arg, cb_async);
	}

	err = ccow_get_cont(STREAM_COMP, iov, iovcnt, off, 1, &STREAM_IDX);
	if (err != 0) {
		printf("ccow_get_cont returned error status %d \n", err);
	}
	assert_int_equal(err, 0);
	...
	err = ccow_wait(STREAM_COMP, STREAM_IDX);
...
void
btmt_stream_put(ccow_t tc, char *bid, char *oid, struct iovec *iov, size_t iovcnt,
	uint64_t off, ccow_callback_t cb_async, void *arg)
{
	...
	err = ccow_put_cont(STREAM_COMP, iov, iovcnt, off, 1, &STREAM_IDX);
	if (err != 0) {
		printf("ccow_put_cont returned error status %d \n", err);
	}
	assert_int_equal(err, 0);
...
sbtm_put(void **state, uint64_t offset, size_t size, size_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		sprintf(ptr, "%6.6zu: %6.6zu: %6.6zu: byte me world.",
	...
	btmt_stream_put(cl, STRESS_TEST_BID, SBTM_TEST_OID, iov, iovcnt,
	    offset, NULL, NULL);
	...
	size_t exp_count)
{
	...
	btmt_stream_get(cl, STRESS_TEST_BID, SBTM_TEST_OID, iov, iovcnt,
	        offset, NULL, NULL, NULL, 0);

	char tmp[size];

	for (i = 0; i < count; i++) {
		sprintf(tmp, "%6.6zu: %6.6zu: %6.6zu: byte me world.",
		    (offset + (size * i)),
		    size, exp_count);
...
sbtm_do_puts(void **state)
{
	...
	if (STREAM_IDX != 0) {
		int err = ccow_finalize(STREAM_COMP, NULL);
		if (err != 0) {
			printf("ccow_finalize returned error status %d \n", err);
		}
		assert_int_equal(err, 0);
...
main(int argc, char **argv)
{
	 ...
	 */
	while ((opt = getopt(argc, argv, "hc:i:m:o:rst:vn")) != -1) {
		switch (opt) {
