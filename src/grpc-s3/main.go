...
func (s *Server) Listen() error {
	...
	log.Printf("S3 controller in version %v serving on %v is ready for gRPC clients", release, s.bind)
	return gs.Serve(ln)
}
...
func InitGrpc(ipnport, svc string) {
	...
	go func() {
		...
		if err != nil {
			log.Fatalf("Failed to launch the EFSS3 due to %v", err)
		}
	}()
}

func InitS3(svc string, fgMode bool) {
	...
	if strings.Compare(stype, "s3") != 0 {
		log.Fatalf("Wrong service type %s != s3 for service %s", stype, svc)
	}
	...
	if p := os.Getenv("EFSS3_HTTP_PORT"); p != "" {
		httpPort = p
	}

	httpsPort := "8443"
	httpsPort, _ = efsutil.GetMDKey("", "svcs", svc, "", "X-HTTPS-Port")
	if p := os.Getenv("EFSS3_HTTPS_PORT"); p != "" {
		httpsPort = p
	}

	for _, v := range keys {
		s := strings.Split(v, "/")
		log.Printf("Starting s3gw for svc=%s on ports %s and %s, %v\n", svc, httpsPort, httpPort, s)
		InitCmd(svc, "s3gw", []string{"-cluster", s[0], "-tenant", s[1], "-port", httpPort, "-sport", httpsPort})
		break
	}
